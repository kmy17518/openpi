...
| 5. + MLP activations kept instead of recomputed (needs ≤ ~128 samples per GPU: measured with gradient accumulation over 4 microbatches, since removed; on 4 GPUs the per-GPU batch is 128 anyway) | 7.60 s | 1.36× | `OPENPI_REMAT_POLICY=save_mlp` |

Where the time went in the baseline (see the roofline analysis in `/tmp/dev/profiling/runs/*/roofline.md`): 55 % GEMMs at 60 % of the achievable bf16 peak, 24 % XLA's unfused attention (f32 logits materialised and shuffled through three layout-transpose kernels running at 0.6-2 TB/s), 15 % other memory-bound fusions, and a full forward recompute of every layer in the backward pass (`nothing_saveable`). The JAX upgrade alone fixes the transposes (XLA 0.11 emits them properly) and knows the GPU (no `sm_101` fallback, Triton GEMMs would work again). Fused attention then removes the remaining softmax traffic. FlashAttention-4 (`flash-attn-4` 4.0.0b31, dedicated head-dim-256 Blackwell kernels, called from the jitted program through an XLA-FFI→Python→torch bridge, pi0's block mask realised as two packed-varlen calls per layer) measured exactly the same 8.05 s: attention is only ~5 % of the step once the transposes are gone, and FA4's Python round trip per layer plus the pack/unpack gathers for the varlen layout ate what its faster kernels (fwd 4 ms, bwd 13 ms per layer at batch 512 vs. 9/16 ms for the cuDNN variant) saved, while it also broke XLA's command buffers (host dispatch only ~2 s ahead of the GPU instead of ~7.6 s). It was removed again; the cuDNN path is pure JAX. `save_mlp` removes the recompute of the MLP GEMMs (0.7 s of GPU time per 512 samples). It was measured on one GPU with gradient accumulation over 4 microbatches of 128 (code since removed: on the 4-GPU node each GPU sees 128 samples anyway), where the 4× kernel launches made the host thread the limiter for ~0.3 s per step, so the net gain was smaller than the GPU-time saving. Re-enabling XLA's Triton GEMMs (possible again with XLA 0.11) was 1.5 % slower than cuBLAS on this model, so `launch.sh` keeps `--xla_gpu_enable_triton_gemm=false`.

#### JAX 0.11 environment (`.venv-jax`)

`scripts/b1k/setup_venv_jax.sh` creates it (idempotent; `uv`, CPython 3.12 downloaded by uv). It installs the requirements inlined in the script — the direct dependencies of `pyproject.toml` with `jax[cuda13]==0.11.1`, `flax==0.12.9`, `orbax-checkpoint==0.12.4`, `optax==0.2.8`, `chex==0.1.92`, `numpy>=2.1`, and `torch==2.10.0+cu130` / `torchvision==0.25.0+cu130` from PyTorch's cu130 index (CUDA 13 like JAX; torch only runs the data loader on the CPU) — with `uv --override` entries (also inlined) lifting three sets of pins that would otherwise win: lerobot's `numpy<2`, the `[tool.uv] override-dependencies` of `pyproject.toml` (`ml-dtypes`, `tensorstore`, meant for jax 0.5.3; this is also why the script resolves from a scratch directory), and torch's exact pins of older CUDA libraries (cuBLAS 13.1, cuDNN 9.15) in favour of the versions jaxlib 0.11.1 was built against (`nvidia-cublas>=13.8`, `nvidia-cudnn-cu13>=9.26`, `nvidia-cuda-runtime>=13.4`, ...). openpi itself is installed editable with `--no-deps`. Finally it writes a site hook (`openpi_cuda_preload.pth`) that loads those CUDA 13 libraries before `import torch` can load its older copies with the same sonames, and runs a sanity check (versions, a matmul, cuDNN fused attention with head_dim 256, `import openpi`).
...
`OPENPI_REMAT_POLICY` (`openpi.models.gemma.remat_block`, used by the Gemma and SigLIP layer scans): `nothing_saveable` (default; every layer's forward is recomputed in the backward) or `save_mlp` (keep the gating/up GEMM outputs tagged with `checkpoint_name` in `lora.FeedForward`; 2 × [tokens, 16384] bf16 per layer, i.e. 533 GB at batch 512 and 133 GB at microbatch 128). It needs the activation memory of a small per-GPU batch: on a 288 GB B300 it fits at 128 samples per GPU (the default 4-GPU launch with global batch 512) but not at 512. Keeping every activation (no remat) does not fit even at 64 samples per GPU.
