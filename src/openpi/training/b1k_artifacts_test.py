...
def test_statistics_representation_guard_and_explicit_legacy(tmp_path):
    ...
    legacy = factory("b1k/R1Pro-legacy-torso-delta").create(tmp_path, model_config())
...
def test_legacy_torso_output_reconstructs_old_four_joint_delta(tmp_path):
    current = factory().create(tmp_path, model_config())
    legacy = factory("b1k/R1Pro-legacy-torso-delta").create(tmp_path, model_config())
    ...
    old_output = transforms.compose(legacy.data_transforms.outputs)({"state": state, "actions": actions.copy()})
    new_output = transforms.compose(current.data_transforms.outputs)({"state": state, "actions": actions.copy()})
    np.testing.assert_allclose(old_output["actions"][:, 6], 0)
    np.testing.assert_allclose(new_output["actions"][:, 6], -0.15)
...
def test_serve_restores_prompt_and_budget_before_loading_policy(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openpi.shared.xla_gpu_compat.configure_xla_flags", lambda: None)
...
def test_resume_rejects_representation_or_conditioning_change_before_restore(tmp_path):
    ...
    legacy = factory("b1k/R1Pro-legacy-torso-delta").create(tmp_path / "legacy", model_config())
